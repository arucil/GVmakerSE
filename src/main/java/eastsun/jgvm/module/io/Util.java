...
public final class Util {
    ...
    static {
        try {
            InputStream in = null;
            in = Util.class.getResourceAsStream("/gbfont.bin");
            gb12Data = new byte[in.available()];
            readData(in, gb12Data);

            in = Util.class.getResourceAsStream("/gbfont16.bin");
            gb16Data = new byte[in.available()];
            readData(in, gb16Data);

            in = Util.class.getResourceAsStream("/ascii.bin");
            ascii12Data = new byte[in.available()];
            readData(in, ascii12Data);

            in = Util.class.getResourceAsStream("/ascii8.bin");
